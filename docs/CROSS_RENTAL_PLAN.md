...
Status: **built on this branch** (steps 1–6 below, plus the bookings engine the owner asked for in question 8). Follow-up after review: the Allocations tab was retired as redundant; gear is checked out and returned from the accepted quote itself, and each item's bookings show on the equipment list ("Where") and its popup. The README sections "Cross rentals & vendor pricing" and "Bookings (allocations)" are the user-facing description; this document is the design record.
