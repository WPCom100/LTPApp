...
(function() {
  ...
  window.CRMCompanyDetail = function({ ctx }) {
    ...
    return h(window.LTPModal, { title: company.name, onClose: function() { ctx.setSelectedCompanyId(null); }, wide: true },
      ...
      // What this vendor charges us per catalog item (the cross-rental price
      // memory) and what we've rented from them. Only for vendors — a client-
      // only company has nothing to price. See components/vendor-rates.js.
      company.isVendor && ctx.setVendorRates && h("div", { style: { marginBottom: 20 } },
        h("h4", { style: { fontSize: "13px", fontWeight: 700, color: B.textSec, margin: "0 0 10px", textTransform: "uppercase", letterSpacing: "0.06em" } }, "Rental Rates"),
        h(window.VendorRatesEditor, { vendorCompanyId: company.id, equipment: ctx.equipment, companies: ctx.companies,
          vendorRates: ctx.vendorRates, setVendorRates: ctx.setVendorRates }),
        h("h4", { style: { fontSize: "13px", fontWeight: 700, color: B.textSec, margin: "16px 0 10px", textTransform: "uppercase", letterSpacing: "0.06em" } }, "Cross Rentals From This Vendor"),
        h(window.VendorCrossHistory, { vendorCompanyId: company.id, crossRentals: ctx.crossRentals,
          onOpenOrder: function(oid) { window.LTPRouter.navigate("rentals/cross-rentals/" + oid); } })),
