...
(function() {
  ...
  function AddItemPicker({ sectionId, sectionLabel, sectionItems, allSections, onAdd, onClose, equipment, products, services, fees, feeAcct, feePicks, allocations, crossRentals, vendorRates, setCrossRentals, setVendorRates, companies, projects, quoteDates }) {
    ...
    // explicit default, sticks while the description is edited.
    ...
    function addCustomFee() {
      ...
      onAdd({
        ...
        cost: 0, notes: "", taxable: true, qbIncomeAccountId: feeAcctValue || null,
      });
  ...
  function LineItemRow({ item, sectionId, quoteStatus, onUpdate, onDelete, onMove, sortable, itemIndex, itemCount, services, products, equipment, fees, customerTaxable, feeAcct }) {
    var isMobile = window.LTP_useIsMobile();
    ...
    // post by. Rides along to the invoice the quote becomes.
    var feeAcctField = isFee && !item.feeId && feeAcct && h(window.LTPFeeAccountSelect, {
      value: window.LTP_customFeeAccount(item), picker: feeAcct, editable: !isLocked,
  ...
  function SectionBlock({ section, quoteDates, quoteStatus, onLabelChange, onUpdate, onDelete, onAddItem, onItemUpdate, onItemDelete, onItemMove,
                          ...
                          sectionSubtotal, sectionMargin, sectionGear, services, products, equipment, fees, customerTaxable, feeAcct,
                          stale, onBump, onKeep }) {
    var isMobile = window.LTP_useIsMobile();
    var isLocked = quoteStatus === "accepted" || quoteStatus === "converted";
      ...
      h("div", { style: { display: "flex", flexDirection: "column", gap: 4, marginBottom: 8 } },
        ...
        section.items.map(function(it, itIdx) {
          return h(LineItemRow, { key: it.id, item: it, sectionId: section.id, quoteStatus: quoteStatus,
            ...
            services: services, products: products, equipment: equipment, fees: fees, customerTaxable: customerTaxable, feeAcct: feeAcct });
        })
      ),
  ...
  window.QuotesBuilder = function({ quoteId, isNew, quotes, setQuotes, getNextQuoteId, products, services, clientRates, fees, equipment, allocations, crossRentals, vendorRates, setCrossRentals, setVendorRates, companies, contacts, projects, invoices, setInvoices, getNextInvoiceId, settings, isAdmin, qbo }) {
    ...
    window.LTP_useRemoteEdits(
      ...
      String(quoteId) + ":" + String(isNew), "quotes", ["activity"]);
 ...
 changes: [{ cat: "Status", detail: draft.status + " \u2192 draft" }] };
    ...
    return h("div", { style: { display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" } },
      ...
      // Send Quote modal. The action row is the modal's `footer` so a phone
      // pins it above the home indicator instead of burying it under the
      // email; the body is the shared preview + compose layout, which folds the
      // preview into a summary line on a phone (components/doc-email-pane.js).
      showSendModal && h(window.LTPModal, { title: draft.status === "draft" ? "Send Quote" : "Resend Quote", onClose: function() { setShowSendModal(false); }, wide: true,
        ...
        h(window.LTPSendModalBody, {
          ...
          amount: "$" + window.LTP_money((window.LTP_QUOTE_TOTALS(draft) || {}).total || 0),
          // Right: Email preview
          ...
          // Left: Quote preview — the per-section breakdown
